...
import tg_bot.modules.welcomestrings as welcomestrings
...
def set_welcome(bot: Bot, update: Update) -> str:
    ...
    msg.reply_text("Added a Custom message as your Welcome message \n Use /welcome to Show your current Welcome Message")
...
@user_admin
@loggable
def random_welcome(bot: Bot, update: Update) -> str:
    chat = update.effective_chat  
    user = update.effective_user  
    msg = update.effective_message.reply_text(random.choice(welcomestrings.WELCOME_RANDOM_STRINGS))
    ...
    if chat.type == chat.PRIVATE:
        ...
        update.effective_message.reply_text("To Set Random Welcome , Add me to a Group with all Admin Permissions. \n Then Run /randomwelcome In Group",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text=" Add me to a Group ",url="t.me/{}?startgroup=true".format(bot.username))]]))
      


    sql.set_custom_welcome(chat.id, content or text, data_type, buttons)
    msg.reply_text("Random Welcome Message successfully Added")
...
- /welcome noformat: Shows current Welcome settings without formatting 
- /setwelcome <sometext>: Set up a custom welcome message. If replying to the media is used, that media is used. 
- /setgoodbye <text> : Setup a custom goodbye message
- /resetwelcome: Go back to the default welcome message ...
- /resetgoodbye : Rollback to Default Goodbye String.
- /clearjoin <on / off>: Delete the message when * Member * joins .. 
- /cleanwelcome <on / off>: On the new member, try deleting the previous welcome message to avoid chat spam.
- /welcomehelp: For more welcome formatting help.
...
RANDOM_WELCOME = CommandHandler("randomwelcome", random_welcome, filters=Filters.group)
...
dispatcher.add_handler(RANDOM_WELCOME)
